...
<script lang="ts">
  ...
  function addPerson(event: SubmitEvent) {
    ...
    let joinDate = form.get('joining-date')
      ? new Date(form.get('joining-date') as string)
      : new Date();
...
<form
  ...
  <input
    on:invalid={(event) => event.currentTarget.setCustomValidity('Invalid DOB')}
    type="text"
    placeholder="Date of birth"
    on:focus={(event) => (event.currentTarget.type = 'date')}
    name="dob"
    class="col-span-1 rounded-md"
    max={getDateStringMinusYears(17)}
    min={getDateStringMinusYears(60)} />
  ...
  {#if studentForm}
    <select name="department" class="col-span-1 rounded-md">
